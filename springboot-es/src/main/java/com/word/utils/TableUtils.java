...
public class TableUtils {
    ...
    public static void main(String[] args) throws Exception {
        //parseDocx("/Users/hujh/Downloads/表格案例1.docx");

        //读取图片
        doxc();
    }

    public static void parseDocx(String filePath) {
        ...
        try {
            ...
            if (filePath.toLowerCase().endsWith("docx")) {
                ...
                while(it.hasNext()){
                    ...
                    TableUtils wordUtils = new TableUtils();
                    String tableTag = wordUtils.readTable(table);
    ...
    public String readTable(XWPFTable table) throws IOException {
        ...
        StringBuilder tableToHtmlStr = new StringBuilder("<table style=\"border:0.5px solid #000;border-collapse: collapse;\">");

        for (int i = 0; i < tableRowsSize; i++) {
            tableToHtmlStr.append("<tr style=\"border:0.5px solid #000;\">");
            ...
            for (int j = 0; j < tableCellsSize; j++) {
                ...
                String backgroundColor = tableCell.getColor();//单元格的背景色
                // 获取单元格的属性
                CTTcPr tcPr = tableCell.getCTTc().getTcPr();
                int colspan = getColspan(tcPr);
                if (colspan > 1) { // 合并的列
                    tableToHtmlStr.append("<td style=\"background-color: #"+backgroundColor+";border:0.5px solid #000;\" colspan='" + colspan + "'");
                } else { // 正常列
                    tableToHtmlStr.append("<td style=\"background-color: #"+backgroundColor+";border:0.5px solid #000;\" ");
                }
     ...
     * 读取图片
     */
    public static void doxc() throws Exception {
        String importPath = "/Users/hujh/Downloads/表格案例1.docx";
        String absolutePath = "/Users/hujh/Downloads/";
        try {
